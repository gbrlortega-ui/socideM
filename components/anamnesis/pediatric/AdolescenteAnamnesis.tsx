...
export default function AdolescenteAnamnesis({ onBack }: Props) {
  const [currentStep, setCurrentStep] = useState<FormStep>(FormStep.IDENTIFICATION);
  
  const [record, setRecord] = useState<ClinicalRecord>({
    id: { nome: '', idade: '12-18 anos', sexo: '', cor: '', estadoCivil: 'Solteiro(a)', profissao: 'Estudante', ocupacao: 'Ensino Médio', naturalidade: '', procedencia: '', residencia: '', nomeMae: '', responsavel: '', religiao: '', planoSaude: '' },
    patientType: PatientType.PEDIATRIC,
    pediatricSubType: PediatricSubType.ADOLESCENT,
    qd: '',
    hma: '',
    isda: {},
    antecedentes: {},
      ...
      <div className="flex-1 p-6 lg:p-12 overflow-y-auto">
        <header className="max-w-[1000px] mx-auto w-full flex items-center justify-between mb-8 no-print">
          ...
          <div className="text-right">
            <h3 className="text-2xl font-black text-slate-900 uppercase">Roteiro Adolescente</h3>
        ...
        <main className="max-w-[1000px] mx-auto w-full glass-card p-8 lg:p-14 rounded-[3rem] border border-slate-200 shadow-soft mb-12 bg-white">
          ...
          {currentStep === FormStep.QD_HMA && <HistorySection qpd={record.qd || ''} hma={record.hma || ''} patientType={PatientType.PEDIATRIC} subType={PediatricSubType.ADOLESCENT} onChange={(updates) => setRecord({ ...record, qd: updates.qpd ?? record.qd, hma: updates.hma ?? record.hma })} />}
          ...
          {currentStep === FormStep.ANTECEDENTS_FISIO && (
            <div className="space-y-8 animate-fade-in">
              <h2 className="text-xl font-black uppercase border-b-2 border-slate-100 pb-2 text-slate-800">Desenvolvimento e Puberdade</h2>
              <div className="space-y-6">
                <div>
                  <label className={labelClass}>Desenvolvimento Puberal (Estágios de Tanner)</label>
                  <textarea className={textareaClass} value={record.antecedentes.desenvSexual} onChange={e=>setRecord({...record, antecedentes: {...record.antecedentes, desenvSexual: e.target.value}})} placeholder="Mamas/Gônadas (M/G), Pelos (P), idade da menarca, espermarca, telarca..." />
                </div>
                <div>
                  <label className={labelClass}>Histórico do Crescimento</label>
                  <textarea className={textareaClass} value={record.antecedentes.dnpm} onChange={e=>setRecord({...record, antecedentes: {...record.antecedentes, dnpm: e.target.value}})} placeholder="Estirão puberal, estatura final estimada, comparação com pares..." />
                </div>
                <div>
          ...
          {currentStep === FormStep.ANTECEDENTS_PATO && (
            <div className="space-y-6 animate-fade-in">
              <h2 className="text-xl font-black uppercase border-b-2 border-slate-100 pb-2 text-slate-800">Rastreio Psicossocial (HEEADSSS)</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="md:col-span-2">
                  <label className={labelClass}>Home, Education, Eating, Activities, Drugs, Sexuality, Suicide, Safety</label>
                  <textarea className={textareaClass} value={record.antecedentes.psicossocial} onChange={e=>setRecord({...record, antecedentes: {...record.antecedentes, psicossocial: e.target.value}})} placeholder="Relacionamento familiar, desempenho escolar, hábitos alimentares, amizades, uso de substâncias, sexualidade, humor e ideação, segurança..." />
                </div>
                <div><label className={labelClass}>Doenças e Cirurgias</label><textarea className={textareaClass} value={record.antecedentes.doencasGraves} onChange={e=>setRecord({...record, antecedentes: {...record.antecedentes, doencasGraves: e.target.value}})} /></div>
                <div><label className={labelClass}>Vacinação e Alergias</label><textarea className={textareaClass} value={record.antecedentes.imunizacoes} onChange={e=>setRecord({...record, antecedentes: {...record.antecedentes, imunizacoes: e.target.value}})} placeholder="HPV, Meningo ACWY, dTpa..." /></div>
              </div>
            </div>
          )}

          {currentStep === FormStep.HABITS && <HabitsSection data={record.habitos} onChange={(habitos)=>setRecord({...record, habitos})} />}
          {currentStep === FormStep.PHYSICAL_EXAM && <PhysicalExamSection data={record.exameFisico} patientType={PatientType.PEDIATRIC} subType={PediatricSubType.ADOLESCENT} age={record.id.idade} sex={record.id.sexo} onChange={(exameFisico) => setRecord({...record, exameFisico})} />}
