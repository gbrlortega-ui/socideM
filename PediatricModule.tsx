...
const PEDIATRIC_SECTIONS = [
  {
    title: "Calculadoras pediátricas",
    subtitle: "Crescimento e Nutrição",
    description: "Apoio clínico focado em faixas etárias desde o neonato até a adolescência.",
    tools: [
      { category: "Antropometria", status: "Em breve", title: "Z-score OMS", description: "Interpretação automática de peso, estatura e IMC para idade conforme padrões mundiais.", icon: "monitoring" },
      { category: "Medicamentos", status: "Em breve", title: "Cálculo por Peso (mg/kg)", description: "Segurança na prescrição pediátrica com checagem de doses máximas permitidas.", icon: "medication" }
    ]
  }
];
      ...
      <header className="flex flex-col gap-5">
        ...
        <div className="space-y-3">
          ...
          <p className="text-slate-800 text-xl max-w-3xl font-bold leading-relaxed">
            Protocolos SBP e calculadoras de crescimento integradas para o internato.
          </p>
        </div>
      </header>
        ...
        <section key={idx} className="flex flex-col gap-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {section.tools.map((tool, tIdx) => (
              <div
                key={tIdx}
                className="group relative bg-white border-2 border-slate-300 p-8 rounded-[2.5rem] transition-all duration-300 shadow-card hover:shadow-strong hover:border-rose-600/50 cursor-wait"
              >
                <div className="flex flex-col gap-6">
                  <div className="flex justify-between items-start">
                    <div className="size-14 rounded-2xl bg-slate-100 flex items-center justify-center text-rose-600 border-2 border-slate-300">
                  ...
                  <div>
                    <span className="text-[11px] font-black uppercase tracking-[0.2em] text-rose-600 block mb-1">
                      {tool.category}
                    </span>
